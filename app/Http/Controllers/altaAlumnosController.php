...
class altaAlumnosController extends Controller
{
    ...
    {     
  	...
  	{   
		try{
			...
			{     
				...
				if($alumno_ya_existe==false){            
					Alumno::updateOrCreate(['no_control' => $row[1]], ['nombre' =>$row[2]],['password' => bcrypt($row[3])] ,['carrera'=>$row[5]],['status'=>'pendiente']);
				}   	                      
			}   
        ...
        {    
            Alert::error('Error','Estos alumnos ya se encuentran registrados en el sistema');
