...
<head>
	...
	<script type="text/javascript">
		function mensaje(){
			jError("Se realizo el respaldo", "Mensaje");
...
<body>
	<div class="container">
		...
		<div class="topbar">
			<a><span><i>Inicio / Base de Datos / Respaldo</i></span></a>
		</div>
		
	</div>
	<?PHP
		require '../conexion/conex_mysql.php';
		$backup_file = "../backups/" .$database. "-" .date("d-m-Y_H-i-s"). ".sql";
		$commands = array("mysqldump --opt -h $hostname -u $username -p$password -v $database > $backup_file","bzip2 $backup_file");
