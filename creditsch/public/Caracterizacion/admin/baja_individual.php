<html> 
	<head>
	   ...
	   <link rel="stylesheet" type="text/css" href="../css/jquery.alerts.css">
		...
		<script type='text/javascript'> 
			function alertar() {
				jError('El Registro No Existe', 'Mensaje');
			}
		</script>
		<script type='text/javascript'>
			function preguntar($fic){
				jConfirm('¿Desea Eliminar Registro Del Alumno?','Mensaje',function (respuesta)
				{ 
					if (respuesta)
					{	
						jError('Registro Eliminado','Mensaje');
			 			//window.location.href="delete.php?no_ficha="+$fic; Descomentar para redireccionar y eliminar registro. y subir al servidor el documento php.
					}
				});			
			}
		</script>
		<script type="text/javascript">
			$(document).ready(function()
			{
				$("#buscar").click(function () {
					...
					var $ficha =($('input:text[name=no_ficha]').val());
   ...
   <body>
   	<div class='container'>
		...
		<div class='topbar'>
			<a><span><i>Inicio / Baja / Individual</i></span></a>
			<span class='right_ab'>
				<a href='bajas.php'><strong> Regresar</strong></a>
			</span>
		</div>
    	<section class='tabss'>
			<fieldset> 
				<form name='buscar_ficha' method='POST' action='' autocomplete='off'>
		    		<table width='675' align='center'>
						<tr>
							<td width='380'><stroke><b>No Ficha: </b></td>
							<td></td>
						</tr>
						<tr>
							<td><input type='text' class='solo-numero' name='no_ficha' id='no_ficha' required></td>
							<td><input type='submit' value='Buscar' name='buscar' id='buscar'/></td>
						</tr>
					</table>
				</form>	
			</fieldset>";
	...
	if (isset($_POST['no_ficha'])){ 
		...
		$nombre=utf8_encode($rows['dp_nombre']);
		$apaterno=utf8_encode($rows['dp_ap_paterno']);
		$amaterno=utf8_encode($rows['dp_ap_materno']);
		$sexo=utf8_encode($rows['dp_sexo']);
		$control=utf8_encode($rows['se_no_control']);
		
		echo "
			<script languaje='javascript'>preguntar($ficha);</script>
			<br>
			<div class='contenedor-1'>
				<fieldset><legend><stroke><b>PERSONALES</b></stroke></legend>
